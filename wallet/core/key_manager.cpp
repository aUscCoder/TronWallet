...
bytes KeyManager::encryptUseEncryptedKey(bytesConstRef _v, Address const& _address)
{
    bytes ret;
    auto pass = getPassword(uuid(_address));
    if (!pass.empty())
        ret = m_store.encryptUseEncryptedKey(_v, _address, pass);
    return ret;
}

std::string KeyManager::decryptUseEncryptedKey(bytesConstRef _v, Address const& _address)
{
    std::string ret;
    auto pass = getPassword(uuid(_address));
    if (!pass.empty())
    {
        auto decrypted = m_store.decryptUseEncryptedKey(_v, _address, pass);
        auto decrypted_ref = decrypted.ref();
        ret = std::string((char *)decrypted_ref.data(), decrypted_ref.size());
    }
    return ret;
}
...
void KeyManager::clear(Address const& _a)
{
    auto id = m_addrLookup[_a];
    m_uuidLookup.erase(id);
    m_addrLookup.erase(_a);
