...
// Window Text Functions
	...
	std::wstring GetWindowText() const
	{
		...
		if (iLen <= 0)
			return L"";
		std::wstring str(iLen, ' ');
		::GetWindowText(m_hWnd, (LPWSTR)str.c_str(), iLen + 1);
		return str;
	}
	...
	std::wstring GetDlgItemText(int nID) const
	{
		...
		if (hItem != NULL)
		{
			...
			if (nLength < 512)
				...
				pszText = new wchar_t[nLength + 1];

			::GetWindowText(hItem, pszText, nLength + 1);
			strText = pszText;
			if (pszText != szBuff)
				delete pszText;
		}
...
#define DECLARE_WND_CLASS_WITH_BRUSH(WndClassName, style, brush) \
	static ATL::CWndClassInfo& GetWndClassInfo() \
	{ \
		static ATL::CWndClassInfo wc = \
		{ \
			{ sizeof(WNDCLASSEX), style, StartWindowProc, \
			0, 0, NULL, NULL, NULL, (HBRUSH)(brush), NULL, WndClassName, NULL }, \
			NULL, NULL, IDC_ARROW, TRUE, 0, _T("") \
	}; \
		return wc; \
	}
...
// added by TuotuoXP, investigate message processing time
#ifdef _DEBUG
__declspec(selectany) int g_iMessageProcessingTimeIndex = ::TlsAlloc();
#endif
...
LRESULT CALLBACK CWindowImplBaseT< TBase, TWinTraits >::WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	MSG *pMaxTimeMsg = (MSG*)::TlsGetValue(g_iMessageProcessingTimeIndex);
	if (pMaxTimeMsg == NULL)
	{
		pMaxTimeMsg = new MSG;
		memset(pMaxTimeMsg, 0, sizeof(MSG));
		::TlsSetValue(g_iMessageProcessingTimeIndex, pMaxTimeMsg);
	}
	if (dwTick > pMaxTimeMsg->time)
	{
		*pMaxTimeMsg = msg;
		pMaxTimeMsg->time = dwTick;
	}
#endif
