...
#define FORWARD_MSG_TO_WND_MEMBER(msg, member) \
	if (uMsg == msg) \
	{ \
		_ATL_MSG atlmsg(hWnd, uMsg, wParam, lParam); \
		const _ATL_MSG *pOld = member.m_pCurrentMsg; \
		member.m_pCurrentMsg = &atlmsg; \
		member.ProcessWindowMessage(hWnd, msg, wParam, lParam, lResult); \
		member.m_pCurrentMsg = pOld; \
		return TRUE; \
	}
...
#define CHAIN_MSG_TO_MEMBER(msg, member) \
	if (uMsg == msg) \
	{ \
		if (member.ProcessWindowMessage(hWnd, msg, wParam, lParam, lResult)) \
			return TRUE; \
	}
...
#define ACQUIRE_HWND(left_value) \
	if ((left_value) == NULL) \
	{ \
		(left_value) = hWnd; \
	}
