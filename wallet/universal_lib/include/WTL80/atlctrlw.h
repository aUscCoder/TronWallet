...
class ATL_NO_VTABLE CCommandBarCtrlImpl : public ATL::CWindowImpl< T, TBase, TWinTraits >, public CCustomDraw<T>
{
public:
...
// Methods
	...
	BOOL AttachMenu(HMENU hMenu)
	{
		...
		T* pT = static_cast<T*>(this);
		SetButtonSize(1, pT->GetButtonHeight());
...
// Parent window message handlers
	...
	LRESULT OnParentCustomDraw(int idCtrl, LPNMHDR pnmh, BOOL& bHandled)
	{
		...
		if(pnmh->hwndFrom == m_hWnd)
		{
			...
			{
				T* pT = static_cast<T*>(this);
				lRet = pT->OnItemPrePaint(idCtrl, (LPNMCUSTOMDRAW)lpTBCustomDraw);
				bHandled = TRUE;
			}
		}
		return lRet;
...
// Implementation - internal message helpers
	...
	{
		static UINT uAutoPopupMessage = ::RegisterWindowMessage(_T("WTL_CmdBar_InternalAutoPopupMsg"));
		return uAutoPopupMessage;
	}

	static UINT GetGetBarMessage()
	{
		static UINT uGetBarMessage = ::RegisterWindowMessage(_T("WTL_CmdBar_InternalGetBarMsg"));
	...
	void UpdateRebarBandIdealSize()
	{
		...
		for(int i = 0; i < nCount; i++)
		{
			...
			if(rbi.hwndChild == m_hWnd)
			{
				...
				{
					RECT rect = { 0 };
					GetRectByIndex(nBtnCount - 1, &rect);
