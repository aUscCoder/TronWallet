...
//		CWindowDC dc(::GetDesktopWindow());
	...
	void CreateMemDC(int iWidth, int iHeight, bool bUseDIB = true)
	{
		m_iWidth = iWidth;
		m_iHeight = iHeight;
		m_bUseDIB = bUseDIB;
		if (m_bUseDIB)
		{
			BITMAPINFOHEADER bmih = { sizeof(BITMAPINFOHEADER), m_iWidth, m_iHeight, 1, 32, BI_RGB };
			m_bmpBuffer.CreateDIBSection(NULL, (BITMAPINFO *) &bmih, 0, (VOID**)&m_pBits, NULL, 0);
		}
		else
		{
			CWindowDC dc(::GetDesktopWindow());
			m_bmpBuffer.CreateCompatibleBitmap(dc, m_iWidth, m_iHeight);
		}
	...
	bool BitmapIsNull() const
	{
		return m_bmpBuffer.IsNull();
	}
	...
	void SetData(BYTE data)
	{
		...
		memset(m_pBits, data, m_iWidth * m_iHeight * 4);
	}
	...
	void Create(HDC hDC, int w, int h)
	{
		if (w != m_w || h != m_h)
		{
			...
			if (m_w > 0 && m_h > 0)
			{
				...
				BITMAPINFOHEADER bmih = { sizeof(BITMAPINFOHEADER), m_w, m_h, 1, 32, BI_RGB };
				BYTE *pBits; 
				m_bitmap.CreateDIBSection(NULL, (BITMAPINFO*)&bmih, 0, (VOID**)&pBits, NULL, 0);
