...
template<typename CHAR>
void AppendRawString(const CHAR* source, int length, CanonOutput* output);

template<>
void AppendRawString(const char* source, int length,
  CanonOutput* output) {
  for (int i = 0; i < length; i++) {
    output->push_back(static_cast<char>(source[i]));
  }
...
template<>
void AppendRawString(const wchar_t* source, int length,
  CanonOutput* output) {
  for (int i = 0; i < length; i++) {
    if (!IsQueryChar(static_cast<unsigned char>(source[i])))
      AppendChar(static_cast<unsigned char>(source[i]), output);
    else  // Doesn't need escaping.
      output->push_back(static_cast<char>(source[i]));
  }
}
...
void DoConvertToQueryEncoding(const CHAR* spec,
  ...
  if (IsAllASCII<CHAR, UCHAR>(spec, query)) {
    ...
    if (converter) {
      if (converter->TreatAsNull()) {
        // gaochaohua : deal with url from IE core
        AppendRawString(&spec[query.begin], query.len, output);
      } else {
