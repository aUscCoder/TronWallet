...
class CharsetConverter {
 public:
  ...
  //added by gaochaohua
  //to deal with url with chinese, e.g. http://www.ncxfdc.com.cn/spfgs/xminfo.aspx?项目编号=132
  //because UTF8 converted to UTF16 maybe fail, it leads to call ConvertFromUTF16 to deal with wrong url
  virtual bool TreatAsNull() { return false; }
};
...
const char* RemoveURLWhitespace(const char* input, int input_len,
                                           ...
                                           int* output_len);
const base::char16* RemoveURLWhitespace(
...
bool IDNToASCII(const base::char16* src,
                           int src_len,
                           CanonOutputW* output);
