...
void GURL::InitCanonical(const STR& input_spec, bool trim_path_end) {
  ...
  if (is_valid_ && SchemeIsFileSystem()) {
    if (inner_url_)
      delete inner_url_;
    inner_url_ = new GURL(spec_.data(), parsed_.Length(),
...
GURL GURL::ResolveWithCharsetConverter(
  ...
  if (result.SchemeIsFileSystem()) {
    if (result.inner_url_)
      delete result.inner_url_;
    result.inner_url_ = 
...
bool GURL::HostIsLocalHost() const {
  if (!is_valid_ || spec_.empty())
    return false;
...
void GURL::Swap(GURL* other) {
  ...
  GURL* temp = inner_url_;
  inner_url_ = other->inner_url_;
  other->inner_url_ = temp;
}
