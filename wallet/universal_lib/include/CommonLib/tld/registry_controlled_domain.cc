...
#include "stdafx.h"
#include "registry_controlled_domain.h"
      ...
      if (type & kExceptionRule) {
        if (next_dot == std::string::npos) {
          ...
          // disallow that case, so this kind of rule is invalid.
            assert(0 && "Invalid exception rule");
          return 0;
        }
...
std::string GetDomainAndRegistryImpl(
  ...
  if (registry_length > (host.length() - 2)) {
    assert(0 && "Host does not have at least one subcomponent before registry!");
    return std::string();
  }
...
std::string GetDomainAndRegistry(
  ...
  const url_parse::Component host = gurl.parsed_for_possibly_invalid_spec().host;
...
bool SameDomainOrHost(
  ...
  const url_parse::Component host1 = gurl1.parsed_for_possibly_invalid_spec().host;
  const url_parse::Component host2 = gurl2.parsed_for_possibly_invalid_spec().host;
