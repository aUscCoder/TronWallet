...
inline void* GetProcAddr(LPCWSTR lpszDLL, LPCSTR lpszEntry)
{
	HMODULE hModule = ::GetModuleHandle(lpszDLL);
	if (!hModule)
		hModule = ::LoadLibraryW(lpszDLL);
	if (hModule)
		return (void*)::GetProcAddress(hModule, lpszEntry);

	return NULL;
}


#define TOUCH_API(DLL, NAME, ARGS, CALLARGS) \
	typedef BOOL (WINAPI * _t##NAME)ARGS;\
	_declspec(selectany) _t##NAME __f##NAME = (_t##NAME)-1;\
	static BOOL NAME ARGS \
	{\
		if (__f##NAME == (_t##NAME)-1)\
			__f##NAME = (_t##NAME)GetProcAddr(DLL, #NAME);\
		if (__f##NAME)\
			return __f##NAME CALLARGS;\
		return FALSE;\
	}

TOUCH_API(L"user32.dll", InitializeTouchInjection, (UINT maxCount, DWORD dwMode), (maxCount, dwMode));
TOUCH_API(L"user32.dll", InjectTouchInput, (UINT count, const POINTER_TOUCH_INFO* contacts), (count, contacts));
TOUCH_API(L"user32.dll", GetPointerType, (UINT pointerId, POINTER_INPUT_TYPE* pointerType), (pointerId, pointerType));
TOUCH_API(L"user32.dll", GetPointerCursorId, (UINT pointerId, UINT* cursorId), (pointerId, cursorId));
TOUCH_API(L"user32.dll", GetPointerInfo, (UINT pointerId, POINTER_INFO* pointerInfo), (pointerId, pointerInfo));
TOUCH_API(L"user32.dll", GetPointerInfoHistory, (UINT pointerId, UINT* enteriesCount, POINTER_INFO* pointerInfo), (pointerId, enteriesCount, pointerInfo));
TOUCH_API(L"user32.dll", GetPointerFrameInfo, (UINT pointerId, UINT* pointerCount, POINTER_INFO* pointerInfo), (pointerId, pointerCount, pointerInfo));
