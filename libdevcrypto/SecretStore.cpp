...
bytes SecretStore::encryptUseEncryptedKey(bytesConstRef _v, Address const& _address, std::string const& _pass)
{
    bytes cipherText;
    if (auto k = key(_address))
    {
        ...
        {
            js::mValue ov;
            js::read_string(k->second.encryptedKey, ov);
            o = ov.get_obj();
        }
        ...
        if (!derivedKey.empty())
        {
            if (o["cipher"].get_str() == "aes-128-ctr")
            {
                ...
                cipherText = encryptSymNoAuth(key, iv, _v);
                if (cipherText.empty())
...
bytesSec SecretStore::decryptUseEncryptedKey(bytesConstRef _v, Address const& _address, std::string const& _pass)
{
    bytesSec ret;
    if (auto k = key(_address))
    {
        ...
        if (!derivedKey.empty())
        {
            if (o["cipher"].get_str() == "aes-128-ctr")
            {
                auto params = o["cipherparams"].get_obj();
                h128 iv(params["iv"].get_str());
                ret = decryptSymNoAuth(SecureFixedHash<16>(derivedKey, h128::AlignLeft), iv, _v);
            }
        }
    }
    return ret;
