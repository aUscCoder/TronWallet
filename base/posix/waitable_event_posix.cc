...
void WaitableEvent::Reset() {
  std::lock_guard<std::mutex> locked(kernel_->lock_);
...
*/
  ...
  for (;;) {
    ...
    std::unique_lock <std::mutex> lck(kernel_->lock_);
    if (finite_time) {
      const TimeDelta max_wait(end_time - current_time);
      sw.cv()->wait_for(lck, std::chrono::microseconds(max_wait.InMicroseconds()));
    } else {
      sw.cv()->wait(lck);
    }
  }
...
*/
  ...
  sw.lock()->lock();
    ...
    for (;;) {
      if (sw.fired())
        break;
      std::unique_lock <std::mutex> lck(*sw.lock());
      sw.cv()->wait(lck);
    }
  sw.lock()->unlock();
