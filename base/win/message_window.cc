...
static MessageWindow::WindowClass g_window_class;
...
bool MessageWindow::DoCreate(MessageCallback message_callback,
  ...
  window_ = CreateWindowW((LPCWSTR)MAKEINTATOM(g_window_class.atom()), window_name, 0, 0, 0,
...
LRESULT CALLBACK MessageWindow::WindowProc(HWND hwnd,
  ...
  if (self) {
    ...
    if (self->message_callback_(message, wparam, lparam, &message_result))
