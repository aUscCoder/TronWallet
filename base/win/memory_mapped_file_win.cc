...
bool MemoryMappedFile::MapFileRegionToMemory(
  ...
  file_.Set(CreateFileW(file_path_.c_str(), access, sharing, NULL,
                        disposition, create_flags, NULL));

  if (!file_.IsValid()) {
    DLOG(ERROR) << "Couldn't open " << base::WideToUTF8(file_path_);
    return false;
  }
  ...
  file_mapping_.Set(::CreateFileMapping(file_.Get(), NULL, flags,
