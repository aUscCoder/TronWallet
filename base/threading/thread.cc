...
thread_local bool lazy_tls_quit_properly = false;
...
bool Thread::StartWithOptions(const Options& options) {
  ...
  // fixed).
  {
    ...
    if (options.joinable) {
      ...
      std::thread(std::bind(&Thread::ThreadMain, this)).detach();
    }
    if (!success) {
...
void Thread::FlushForTesting() {
  ...
  task_runner()->PostTask(FROM_HERE,
                          std::bind(&WaitableEvent::Signal, &done));
  done.Wait();
}
...
void Thread::StopSoon() {
  ...
  task_runner()->PostTask(
      FROM_HERE, std::bind(&Thread::ThreadQuitHelper, this));
}
...
void Thread::Run(RunLoop* run_loop) {
  ...
  DCHECK_EQ(id_, std::this_thread::get_id());
...
#ifndef NDEBUG
  quit_properly = lazy_tls_quit_properly;
#endif
