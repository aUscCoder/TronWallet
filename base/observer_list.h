...
class ObserverListBase
    : public std::enable_shared_from_this<ObserverListBase<ObserverType>> {
 public:
  ...
  class Iter {
   public:
    Iter();
    explicit Iter(std::shared_ptr<ContainerType> list);
    ~Iter();
   ...
   private:
    ...
    size_t clamped_max_index(std::shared_ptr<ContainerType> list) const {
      return std::min(max_index_, list->observers_.size());
    }

    bool is_end() const {
      if (auto list = list_.lock())
        return index_ == clamped_max_index(list);
      return true;
    }
...
template <class ObserverType>
template <class ContainerType>
ObserverListBase<ObserverType>::Iter<ContainerType>::Iter(std::shared_ptr<ContainerType> list)
    : list_(list),
      index_(0),
      max_index_(list->type_ == NOTIFY_ALL ? std::numeric_limits<size_t>::max()
                                           : list->observers_.size()) {
  EnsureValidIndex();
  DCHECK(!list_.expired());
...
bool ObserverListBase<ObserverType>::Iter<ContainerType>::operator==(
  ...
  if (is_end() && other.is_end())
    return true;
  return !list_.owner_before(other.list_) && !other.list_.owner_before(list_) && index_ == other.index_;
}
...
ObserverType* ObserverListBase<ObserverType>::Iter<ContainerType>::GetCurrent()
    const {
  if (auto list = list_.lock()) {
    return index_ < clamped_max_index(list) ? list->observers_[index_] : nullptr;
  }
  return nullptr;
}
...
void ObserverListBase<ObserverType>::Iter<ContainerType>::EnsureValidIndex() {
  if (auto list = list_.lock()) {
    size_t max_index = clamped_max_index(list);
    while (index_ < max_index && !list->observers_[index_])
      ++index_;
  }
}
