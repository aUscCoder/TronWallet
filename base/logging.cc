...
bool g_log_severity = true;
bool g_log_filename = true;
...
#if defined(OS_WIN)
  // On Windows we use the same path as the library.
  wchar_t module_name[MAX_PATH];
  GetModuleFileNameW(CURRENT_MODULE(), module_name, MAX_PATH);
...
// static
std::mutex* LoggingLock::log_lock = nullptr;
// static
...
#if !defined(OS_WIN)
  LoggingLock::Init(settings.lock_log, settings.log_file.c_str());
...
void SetMinLogLevel(const std::string& level)
{
  ...
  for (int i = 0; i < LOG_NUM_SEVERITIES; ++i)
    if (all[i].levelName == level)
    {
      SetMinLogLevel(all[i].level);
      break;
    }
}
...
void SetLogItems(bool enable_process_id, bool enable_thread_id,
                 ...
                 bool enable_severity, bool enable_filename) {
  ...
  g_log_severity = enable_severity;
  g_log_filename = enable_filename;
}
...
#if defined(OS_WIN)
  MessageBoxW(nullptr, base::UTF8ToWide(str).c_str(), L"Fatal error",
...
#endif
*/
  if (g_logging_destination == LOG_NONE)
    return;
  // A little hack.
  if (line_ != -2)
    stream_ << std::endl;
  ...
  if ((g_logging_destination & LOG_TO_SYSTEM_DEBUG_LOG) != 0) {
    ...
    if (log_via_asl) {
      ...
      std::string asl_facility =
          main_bundle_id_cf ? CFStringToSTLStringWithEncodingT<std::string>(main_bundle_id_cf,
...
void LogMessage::Init(const char* file, int line) {
  ...
  if (g_log_thread_id)
    stream_ << std::setfill('0') << std::setw(5) << std::this_thread::get_id() << ':';
  if (g_log_timestamp) {
#ifdef _MSC_VER
    SYSTEMTIME lt;
...
#elif defined(OS_POSIX)
std::string SystemErrorCodeToString(SystemErrorCode error_code) {
  return std::string(base::safe_strerror(error_code));
}
#else
#error Not implemented
#endif  // defined(OS_WIN)
