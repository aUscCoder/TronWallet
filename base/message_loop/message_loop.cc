...
#include "base/posix/message_pump_libevent.h"
#endif
#if defined(OS_FUCHSIA)
#include "base/message_loop/message_pump_fuchsia.h"
#endif
#if defined(OS_ANDROID)
#include "base/android/message_pump_android.h"
#endif
...
MessageLoop*& GetTLSMessageLoop() {
  thread_local static MessageLoop* tls_message_loop = nullptr;
  return tls_message_loop;
}
...
MessageLoop::MessageLoop(std::unique_ptr<MessagePump> pump)
  : MessageLoop(TYPE_CUSTOM, std::bind([&pump] { return std::move(pump); })) {
  BindToCurrentThread();
}
...
MessageLoopForUI::MessageLoopForUI(std::unique_ptr<MessagePump> pump)
    : MessageLoop(TYPE_UI, std::bind([&pump] { return std::move(pump); })) {}
