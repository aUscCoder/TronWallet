...
WaitableEvent::WaitableEvent(ResetPolicy reset_policy,
  ...
  CHECK(kr == KERN_SUCCESS) << "mach_port_construct";
...
void WaitableEvent::Signal() {
  ...
  CHECK(kr == KERN_SUCCESS || kr == MACH_SEND_TIMED_OUT) << "mach_msg";
  ...
  if (use_slow_path) {
    ...
    receive_right->SlowWatchList()->lock.unlock();
...
*/
  ...
  if (kr == KERN_SUCCESS) {
    ...
    CHECK(kr == MACH_RCV_TIMED_OUT) << "mach_msg";
    return false;
  }
}
...
#else
  static bool use_slow_path = true; /*!mac::IsAtLeastOS10_12();*/
#endif
...
*/
  ...
  if (kPrimitive == KQUEUE) {
    ...
    int wait_many = kqueue();
    PCHECK(wait_many != -1) << "kqueue";

    int rv = HANDLE_EINTR(kevent64(wait_many, events.data(), count,
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      auto source =
          base::MakeUnique<DispatchSourceMach>(queue, right->Name(), ^{
    ...
    mac::ScopedMachPortSet port_set;
    {
      ...
      CHECK(kr == KERN_SUCCESS) << "mach_port_allocate";
      port_set.reset(name);
    }
    ...
    CHECK(kr == MACH_RCV_TOO_LARGE) << "mach_msg";
...
bool WaitableEvent::PeekPort(mach_port_t port, bool dequeue) {
  if (dequeue) {
    ...
    if (kr == KERN_SUCCESS) {
      ...
      CHECK(kr == KERN_FAILURE) << "mach_port_peek";
      return false;
    }
  }
}
