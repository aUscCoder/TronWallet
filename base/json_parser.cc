...
// holding std::string references to the input string, avoiding about 2/3rds of
...
// A variant on StringValue that uses std::string instead of copying the string
...
class JSONStringValue : public Value {
 public:
  explicit JSONStringValue(const std::string& piece)
  ...
  bool GetAsString(std::wstring* out_value) const override {
    *out_value = base::UTF8ToWide(string_piece_);
    return true;
  }
  Value* DeepCopy() const override {
    return new StringValue(string_piece_);
  }
  bool Equals(const Value* other) const override {
    std::string other_string;
    return other->IsType(TYPE_STRING) && other->GetAsString(&other_string) &&
        std::string(other_string) == string_piece_;
  }

 private:
  // The location in the original input stream.
  std::string string_piece_;
...
std::unique_ptr<Value> JSONParser::Parse(const std::string& input) {
  std::unique_ptr<std::string> input_copy;
  // If the children of a JSON root can be detached, then hidden roots cannot
  // be used, so do not bother copying the input because std::string will not
  // be used anywhere.
  if (!(options_ & JSON_DETACHABLE_CHILDREN)) {
    input_copy = MakeUnique<std::string>(input);
...
bool JSONParser::StringBuilder::CanBeString() const {
  return !string_;
}
...
Value* JSONParser::ConsumeString() {
  ...
  // to do so, and if the string can be represented by std::string.
  if (string.CanBeString() && !(options_ & JSON_DETACHABLE_CHILDREN))
    return new JSONStringValue(string.AsString());

  if (string.CanBeString())
    string.Convert();
...
bool JSONParser::ConsumeStringRaw(StringBuilder* out) {
  ...
  // StringBuilder will internally build a std::string unless a UTF-16
  // conversion occurs, at which point it will perform a copy into a
  // std::string.
  StringBuilder string(NextChar());

  auto length = end_pos_ - start_pos_;
  int32_t next_char = 0;
  ...
  while (CanConsume(1)) {
    ...
    if (next_char != '\\') {
      ...
      // or with a UTF conversion), so using std::string isn't possible -- force
      // a conversion.
      string.Convert();
      ...
      switch (*NextChar()) {
        ...
        case 'x': {  // UTF-8 sequence.
          ...
          int hex_digit = 0;
          if (!HexStringToInt(std::string(NextChar(), 2), &hex_digit) ||
...
bool JSONParser::DecodeUTF16(std::string* dest_string) {
  ...
  if (!HexStringToInt(std::string(pos_, 4), &code_unit16_high))
    return false;
  ...
  if (CBU16_IS_SURROGATE(code_unit16_high)) {
    ...
    if (!HexStringToInt(std::string(pos_, 4), &code_unit16_low))
      return false;
