...
// should not be sent to other log destinations.
 ...
 // [WuJian] change to std::function so that we can use std::bind
 // typedef bool (*LogMessageHandlerFunction)(int severity,
 //   const char* file, int line, size_t message_start, const std::string& str);
 using LogMessageHandlerFunction = std::function<bool(int severity,
   const char* file, int line, size_t message_start, const std::string& str)>;
...
#define DIRECT_GOOGLE_LOG_ERROR \
  logging::LogMessage(logging::LOG_ERROR)
...
#define DIRECT_GOOGLE_LOG_0 DIRECT_GOOGLE_LOG_ERROR
...
#define DIRECT_LOG_STREAM(severity) DIRECT_GOOGLE_LOG_ ## severity.stream()

#define LOG(severity) LAZY_STREAM(LOG_STREAM(severity), LOG_IS_ON(severity))
#define LOG_IF(severity, condition) \
  LAZY_STREAM(LOG_STREAM(severity), LOG_IS_ON(severity) && (condition))

#define DIRECT_LOG(severity) LAZY_STREAM(DIRECT_LOG_STREAM(severity), LOG_IS_ON(severity))
