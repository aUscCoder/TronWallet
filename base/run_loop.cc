...
thread_local RunLoop::Delegate* tls_delegate = nullptr;
...
base::Closure RunLoop::QuitClosure() {
  ...
  std::weak_ptr<RunLoop> run_loop = shared_from_this();
  OnceClosure closure = std::bind([run_loop] () {
    if (auto run_loop_valid = run_loop.lock()) {
      run_loop_valid->Quit();
    }
  });
  return std::bind(&ProxyToTaskRunner, origin_task_runner_, closure);
}
...
base::Closure RunLoop::QuitWhenIdleClosure() {
  ...
  OnceClosure closure = std::bind([run_loop] () {
    if (auto run_loop_valid = run_loop.lock()) {
      run_loop_valid->QuitWhenIdle();
    }
  });
  return std::bind(
      &ProxyToTaskRunner, origin_task_runner_, closure);
}

// static
bool RunLoop::IsRunningOnCurrentThread() {
  Delegate* delegate = tls_delegate;
...
void RunLoop::AddNestingObserverOnCurrentThread(NestingObserver* observer) {
  DCHECK(tls_delegate);
  CHECK(tls_delegate->allow_nesting_);
  tls_delegate->nesting_observers_.AddObserver(observer);
}
...
RunLoop::ScopedDisallowRunningForTesting::~ScopedDisallowRunningForTesting() {
  DCHECK_EQ(current_delegate_, tls_delegate);
  if (current_delegate_)
