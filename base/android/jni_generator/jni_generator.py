...
class JNIFromJavaSource(object):
  ...
  def __init__(self, contents, fully_qualified_class, options):
    ...
    self.class_content = inl_header_file_generator.GetClassContent()
...
""")
  ...
  def GetClassContent(self):
    """Returns the content of the class file."""
    class_template = Template("""\
// Copyright 2017 The Typany Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
...
class ${GENERATE_CLASS} {
 public:
  ${GENERATE_CLASS}(const base::android::JavaParamRef<jobject>& j_binding_obj);

  $METHOD_DECLARES
  
  static bool Register${GENERATE_CLASS}(JNIEnv* env);
}
$CLOSE_NAMESPACE
...
""")
    values = {
        'SCRIPT_NAME': self.options.script_name,
        'FULLY_QUALIFIED_CLASS': self.fully_qualified_class,
        'GENERATE_CLASS': self.GetMethodDeclaresClass(),
        'METHOD_DECLARES': self.GetMethodDeclaresString(),
        'OPEN_NAMESPACE': self.GetOpenNamespaceString(),
        'CLOSE_NAMESPACE': self.GetCloseNamespaceString(),
    }
    return WrapOutput(class_template.substitute(values))
  ...
  def GetMethodDeclaresClass(self):
    """Returns the code corresponding to class declaration."""
    for native in self.natives:
      if native.type == 'method':
        return native.p0_type
        
    return None
    
  def GetMethodDeclaresString(self):
    """Returns the code corresponding to method declaration."""
    ret = []
    for native in self.natives:
      if native.type == 'method':
        ret += [self.GetNativeDeclare(native)]
    return '\n'.join(ret)
...
""")
  ...
  def GetParamsInDeclaration(self, native):
    ...
    if native.type == 'method':
      ...
      joiner = ',\n    '
    return joiner.join(self.GetJNIFirstParam(native, True) +
  ...
  def GetNativeDeclare(self, native):
    ...
    template = Template("""
  ${RETURN_DECLARATION} ${NAME}(JNIEnv* env, ${PARAMS});
...
${FUNCTION_SIGNATURE} {""")
    template = Template("""
static std::atomic_intptr_t g_${JAVA_CLASS}_${METHOD_ID_VAR_NAME};
${FUNCTION_HEADER}
...
std::atomic_intptr_t g_${JAVA_CLASS}_clazz __attribute__((unused));
...
def GenerateJNIHeader(input_file, output_file, output_class_file, options):
  try:
    if os.path.splitext(input_file)[1] == '.class':
      ...
      if options.generate_class:
        class_content = jni_from_java_source.GetClassContent()
  ...
  if output_file:
    ...
    with open(output_file, 'w') as f:
      f.write(content)
    if options.generate_class:
      with open(output_class_file, 'w') as w:
        w.write(class_content)
...
#  build_utils.AddDepfileOption(option_parser)
  ...
  option_parser.add_option('--generate_class', action='store_true',
                           help='Generate template class as well.')
  ...
  if options.output_dir:
    ...
    output_class_file = os.path.join(os.path.dirname(options.output_dir), root_name) + '_tmpl.h'
    
  GenerateJNIHeader(input_file, output_file, output_class_file, options)
