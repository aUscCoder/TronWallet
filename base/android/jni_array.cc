...
base::android::ScopedJavaLocalRef<jobjectArray> ToJavaArrayOfObjects(
    JNIEnv* env, const jclass& object_clz, size_t size, JavaObjectWriterFunc writer) {
  jobjectArray joa = env->NewObjectArray(size, object_clz, NULL);
  CheckException(env);

  for (size_t i = 0; i < size; ++i) {
    ScopedJavaLocalRef<jobject> item = writer(env, i);
    env->SetObjectArrayElement(joa, i, item.obj());
  }
  return ScopedJavaLocalRef<jobjectArray>(env, joa);
}
...
void JavaArrayOfObjectToObjectVector(JNIEnv* env,
                                     jobjectArray array,
  ...
  size_t len = SafeGetArrayLength(env, array);
  for (size_t i = 0; i < len; ++i) {
    ScopedJavaLocalRef<jobject> item(env,
                                     env->GetObjectArrayElement(array, i));
    reader(env, item.obj());
  }
}
