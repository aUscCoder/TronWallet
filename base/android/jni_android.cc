...
JavaVM* g_jvm = NULL;
base::android::ScopedJavaGlobalRef<jobject> g_class_loader;
...
void InitReplacementClassLoader(JNIEnv* env,
  ...
  g_class_loader.Reset(class_loader);
}

ScopedJavaLocalRef<jclass> GetClass(JNIEnv* env, const char* class_name) {
  jclass clazz;
  if (!g_class_loader.is_null()) {
    ...
    clazz = static_cast<jclass>(
        env->CallObjectMethod(g_class_loader.obj(),
                              g_class_loader_load_class_method_id,
...
jclass LazyGetClass(
  ...
  static_assert(sizeof(intptr_t) >= sizeof(jclass),
                "AtomicWord can't be smaller than jclass");
  intptr_t value = atomic_class_id->load();
  if (value)
    return reinterpret_cast<jclass>(value);
  ScopedJavaGlobalRef<jclass> clazz;
  clazz.Reset(GetClass(env, class_name));
  intptr_t null_aw = NULL;
  bool cas_result = atomic_class_id->compare_exchange_strong(null_aw,
                                                             reinterpret_cast<intptr_t>(clazz.obj()));
  if (cas_result) {
    // We intentionally leak the global ref since we now storing it as a raw
    // pointer in |atomic_class_id|.
    return clazz.Release();
  } else {
    return reinterpret_cast<jclass>(null_aw);
  }
}
...
jmethodID MethodID::LazyGet(JNIEnv* env,
  ...
  static_assert(sizeof(intptr_t) >= sizeof(jmethodID),
                "AtomicWord can't be smaller than jMethodID");
  intptr_t value = atomic_method_id->load();
  if (value)
    return reinterpret_cast<jmethodID>(value);
  jmethodID id = MethodID::Get<type>(env, clazz, method_name, jni_signature);
  atomic_method_id->store(reinterpret_cast<intptr_t>(id));
  return id;
}
...
template jmethodID MethodID::LazyGet<MethodID::TYPE_STATIC>(
    JNIEnv* env, jclass clazz, const char* method_name,
    const char* jni_signature, std::atomic_intptr_t* atomic_method_id);
...
void CheckException(JNIEnv* env) {
  ...
  static std::string local_exception;
...
*/
    local_exception = GetJavaExceptionInfo(env, java_throwable);
  }
